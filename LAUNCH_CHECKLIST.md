...
- **Push local commits AND redeploy on Vercel**: check GitHub Desktop for unpushed commits and push them, then redeploy — I can't push myself. Note from 2026-08-01: the webhook code was updated locally (new `cancelAtPeriodEnd`/`currentPeriodEnd` sync) but a real Stripe event was tested against production *before* this was deployed, confirming the old webhook code silently ignored the new fields instead of erroring — a reminder that "committed" ≠ "live," always confirm a redeploy happened before assuming a fix is active in production
