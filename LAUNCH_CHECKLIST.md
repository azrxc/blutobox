...
- **Still need to rotate**: Backblaze B2 application key, Neon DB password/connection string, Upstash Redis token, Gmail app password, and regenerate `CRON_SECRET`/`AUTH_SECRET` — all of these were also pasted into this chat's history during initial setup and haven't been rotated yet
